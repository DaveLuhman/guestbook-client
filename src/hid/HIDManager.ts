...
// Track if we've determined that no camera is available
let cameraNotAvailable = false;
...
async function checkCameraHealth() {
  // Skip monitoring if we've determined no camera is available
...
function updateDeviceStatusDisplay(status: DeviceStatusResponse) {
  updateDeviceStatusIndicator(
    'barcode',
    status.barcode.connected ? 'connected' : 'disconnected'
  );
  updateDeviceStatusIndicator(
    'msr',
    status.msr.connected ? 'connected' : 'disconnected'
  );
}

// Update individual device status indicator
function updateDeviceStatusIndicator(
  device: 'barcode' | 'msr' | 'camera',
  status: string,
  error?: string
) {
...
function clearDeviceErrorMessage(_device: 'barcode' | 'msr') {
  ...
  if (entryData) {
    ...
    const disconnectedMessage = entryData.querySelector(
      '.device-disconnected-message'
    );
